...
从 GitHub 下载项目压缩包后，解压缩。**注意**：从 GitHub 下载的 zip 文件解压后，通常会有一个外层文件夹（如 `ssrn-quick-jump-main`），你需要选择这个外层文件夹里面的那个包含 `manifest.json`、`content.js` 等文件的文件夹。
...
如果选择错误会提示"清单文件缺失或不可读取"，这时需要重新选择正确的文件夹。
